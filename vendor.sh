#!/usr/bin/env bash
# Builds a minimal static FFmpeg (plus libvpx + libopus + dav1d + zimg) into
# vendor/out. Runs inside the Docker build image — see Dockerfile. Official
# upstream release tarballs only, plus the minimal patches in patches/ (each
# one a few lines, applied verbatim and reviewable in-repo).
#
...
if ! built ffmpeg; then
    ...
    (cd "$SRC/ffmpeg" && \
        ./configure \
            ...
            --enable-encoder=libvpx_vp9,libopus,libwebp,libwebp_anim \
            --enable-muxer=webm,webp \
